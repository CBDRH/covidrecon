...
covid_prepare_estimate_repro <- function(covid_data_changepoint){
  covid_data_changepoint %>%
    ungroup() %>% 
...
tidy_incidence_data <- 
  ...
  nest() %>% 
  mutate(prepared_data = purrr::map(data, covid_prepare_estimate_repro),
...
tidy_incidence_data %>% 
  ...
  unnest(cols = c(result))  %>% 
    select(country_region, date, everything())

tidy_incidence_data %>% 
  select(country_region,
         result) %>% 
  mutate(no_result = map_lgl(result, function(x) nrow(x) == 0)) %>% 
  filter(no_result) %>% 
  select(country_region, no_result)
```
...
estimate_repro_errors <- function(covid_data_estimated){
  covid_data_estimated %>% 
  select(country_region,
         result) %>% 
  mutate(no_result = map_lgl(result, function(x) nrow(x) == 0)) %>% 
...
covid_repro_countries_not_measured <- estimate_repro_errors(covid_estimated_repro)

covid_repro_countries_not_measured

covid_tidy_repro
```

```{r}
countries_repro_error <- estimate_repro_errors(covid_estimated_repro)$country_region
covid_estimated_repro %>% 
  filter(country_region %in% countries_repro_error) %>% 
  select(country_region,
         repro_estimate) %>% 
