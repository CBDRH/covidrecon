...
high_incidence_countries <- high_incidence_countries %>%
  ...
  mutate(incident_cases = case_when(country_region == "Japan" &
...
for (country in unique(high_incidence_countries$country_region)) {
  ts <- high_incidence_countries %>%
  filter(country_region == country) %>%
  pull(incident_cases)

 cpt <- cpt.meanvar(ts,class=FALSE)["cpt"]
 
  changedate <- high_incidence_countries %>%
     filter(country_region == country) %>%
     slice(cpt) %>%
     select(country_region, Date) %>%
     rename(ChangeDate = Date)

  change_dates <- change_dates %>%
    bind_rows(changedate)
}
...
high_incidence_countries %>% 
  filter(country_region %in% c(highlight_countries, "US", "Germany", "Denmark")) %>%
  left_join(change_dates) %>%
  ggplot(aes(x=Date, y=incident_cases)) +
    geom_line() +
    geom_vline(aes(xintercept=ChangeDate)) +
